...
def print_classification_report(gdf):
    ...
    print(f"total non-public LULC spaces: {len(gdf[gdf['is_public'] == 'no']['id_left'])}")
    print("-" * 20)
    print(f"total LULC spaces (area): {total_green_area}")
    print(f"percentage of labeled LULC spaces (area): {round((gdf[~gdf['is_public'].isna()]['area_left'].sum() / total_green_area) * 100, 2)}%")
    print(f"percentage of public LULC spaces (area): {round((gdf[gdf['is_public'] == 'yes']['area_left'].sum() / total_green_area) * 100, 2)}%")
    print(f"percentage of non-public LULC spaces (area): {round((gdf[gdf['is_public'] == 'no']['area_left'].sum() / total_green_area) * 100, 2)}%")
